...
test.describe('Landing Page - E2E Tests', () => {
  ...
  test('completed does not exceed total', async ({ page }) => {
    ...
    const completed = parseInt(await completedEl.getAttribute('data-target') || '0');
  ...
  test('metrics persist after reload', async ({ page }) => {
    ...
    const before = await page.locator('[data-testid="total-challenges"]').getAttribute('data-target');
    
    await page.reload();
    await page.waitForTimeout(2000);
    
    const after = await page.locator('[data-testid="total-challenges"]').getAttribute('data-target');
